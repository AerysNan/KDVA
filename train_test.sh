...
CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/test.py configs/custom/ssd_${CONFIG}.py tmp_${PREFIX}_${POSTFIX}/latest.pth --out tmp_${PREFIX}_${POSTFIX}/result_0.pkl -d ${PREFIX}_${POSTFIX}_test_0
for i in $(seq 1 1 $(expr ${SIZE} - 1))
do
  CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/train.py configs/custom/ssd_${CONFIG}.py --work-dir tmp_${PREFIX}_${POSTFIX}/ --train-dataset ${PREFIX}_${POSTFIX}_train_$(expr ${i} - 1) --val-dataset ${PREFIX}_${POSTFIX}_val_$(expr ${i} - 1)
  cp tmp_${PREFIX}_${POSTFIX}/latest.pth snapshot/models/${PREFIX}_${POSTFIX}/${i}.pth
  CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/test.py configs/custom/ssd_${CONFIG}.py tmp_${PREFIX}_${POSTFIX}/latest.pth --out tmp_${PREFIX}_${POSTFIX}/result_${i}.pkl -d ${PREFIX}_${POSTFIX}_test_${i}
done

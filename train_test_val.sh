...
do
  CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/train.py configs/custom/ssd_${CONFIG}.py --work-dir tmp_${PREFIX}_${POSTFIX}_${CONFIG}/ --train-dataset ${PREFIX}_${POSTFIX}_train_$(expr ${i} - 1) --val-dataset ${PREFIX}_${POSTFIX}_val_$(expr ${i} - 1) --no-test --load-from tmp_${PREFIX}_${POSTFIX}_${CONFIG}/latest.pth --seed 0 --deterministic

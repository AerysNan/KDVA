...
do
  CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/model_train.py configs/custom/ssd_${CONFIG}.py --work-dir tmp_${PREFIX}_${POSTFIX}_${CONFIG}/ --train-dataset ${PREFIX}_${POSTFIX}_train_$(expr ${i} - 1) --no-validate --load-from tmp_${PREFIX}_${POSTFIX}_${CONFIG}/latest.pth --seed 0 --deterministic
...
CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/range_eval.py -r ${PREFIX}_${POSTFIX}_${CONFIG} -d ${PREFIX} -s True -n ${SIZE} --gt True
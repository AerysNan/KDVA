...
do
  ...
  CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/test.py configs/custom/ssd_base.py tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/latest.pth --out tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/$(printf %02d 0).pkl -d ${PREFIX}_${stream}_test_0
done

for i in $(seq 1 1 $(expr ${SIZE} - 1))
do

  CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/train.py configs/custom/ssd_fh.py --work-dir tmp_agg_${PREFIX}_merge$4$5_${POSTFIX}/ --train-dataset merge$4$5_${POSTFIX}_train_$(expr ${i} - 1) --no-validate --load-from tmp_agg_${PREFIX}_merge$4$5_${POSTFIX}/latest.pth --seed 0 --deterministic
  for stream in $4 $5
  do
    cp tmp_agg_${PREFIX}_merge$4$5_${POSTFIX}/latest.pth tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/latest.pth
  done
  ...
  for stream in $4 $5
  do
    CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/train.py configs/custom/ssd_fb.py --work-dir tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/ --train-dataset ${PREFIX}_${stream}_${POSTFIX}_train_$(expr ${i} - 1) --no-validate --load-from tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/latest.pth --seed 0 --deterministic
    cp tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/latest.pth snapshot/models/${PREFIX}_${stream}m$4$5_${POSTFIX}_agg/${i}.pth
  done

  for stream in $4 $5
  do
    CUDA_VISIBLE_DEVICES=${DEVICE} python3 tools/test.py configs/custom/ssd_base.py tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/latest.pth --out tmp_agg_${PREFIX}_${stream}m$4$5_${POSTFIX}/$(printf %02d ${i}).pkl -d ${PREFIX}_${stream}_test_${i}
  done
done

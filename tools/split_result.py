...
parser.add_argument("--path", "-p", help="path to result file", type=str, required=True)
parser.add_argument("--size", "-s", help="size of splitted result", type=int, default=500)
...
os.makedirs(f'{args.root}/{args.output}', exist_ok=True)

for i in range(len(results) // args.size):
    with open(f'{args.root}/{args.output}/{i:02d}.pkl', 'wb') as f:
